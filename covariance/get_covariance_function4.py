...
def get_covariance_function4(x, y, data, noise, delta, fu, f_para, filenamei, ncore):
    '''
    ...
    for k in xrange(1, int(np.ceil(dmax / delta))):
        ...
        for i, j in itertools.product(xrange(len(x)), xrange(len(x))):
            if i != j:
                dij = np.degrees(g.inv(x[i], y[i], x[j], y[j])[2] / float(R)) #great_circle_distance(x[i], y[i], x[j], y[j])
    ...
    for i in xrange(len(li_dict.keys())):
        ...
        for j in xrange(len(li_dict.values()[i])):
            ...
            if type_l == 'e':
                ...
                m = 1
            beta[j, 0] = calc_beta(x[pt1], x[pt2], y[pt1], y[pt2], data[pt1][k], data[pt2][m], noise[pt1][k], noise[pt2][m], type_function(type_k, type_l))
    ...
    for i, j in itertools.product(xrange(len(x)), xrange(len(x))):
        if i != j:
            dij.append(np.degrees(g.inv(x[i], y[i], x[j], y[j])[2] / float(R))) #great_circle_distance(x[i], y[i], x[j], y[j]))
        else:
            dij.append(0)
