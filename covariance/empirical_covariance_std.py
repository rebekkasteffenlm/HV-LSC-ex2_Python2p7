...
def empirical_covariance_std(x, y, data1, data2, noise1, noise2, delta, d_conv, f, filenamei, nr):
    '''
   ...
   if len(np.where(abs(data2) > rms_data2)[0]) > 0:
    ...
    if d_conv == 'sphere':
        ...
        c = []
        b = []
        for key, value in ddata_dict.items():
            mean_i = np.mean(data1[np.unique(value[:,0])])
            mean_j = np.mean(data2[np.unique(value[:,1])])
            std_i = np.std(data1[np.unique(value[:,0])])
            std_j = np.std(data2[np.unique(value[:,1])])
            a = (data1[value[:,0]] - mean_i) * (data2[value[:,1]] - mean_j)
            c.append([(key[0] + key[1]) / 2., np.sum(a) / float(len(a) - 1) / float(std_i * std_j), np.std(a / float(std_i * std_j)), len(a)])
            if (np.sum(a) / float(len(a)) / float(std_i * std_j)) <= 0.5:
                b.append([key[0], key[1]])
        c = np.asarray(c)
        c = c[c[:,0].argsort()]
        cova = c[:,1]
        ...
        if len(b) > 0:
            b = np.array(b)
            b = b[b[:,0].argsort()]
            corr_bounds = np.radians(np.array([b[0,0], b[0,1]])) * R / 1000.
        else:
        ...
        for key, value in ddata_dict.items():
            ...
            std_j = np.std(data2[np.unique(value[:,1])])
            a = (data1[value[:,0]] - mean_i) * (data2[value[:,1]] - mean_j)
            c.append([(key[0] + key[1]) / 2., np.sum(a) / float(len(a) - 1) / float(std_i * std_j), np.std(a), len(a)])
            if (np.sum(a) / float(len(a)) / float(std_i * std_j)) <= 0.5:
                b.append([key[0], key[1]])
        c = np.asarray(c)
        c = c[c[:,0].argsort()]
